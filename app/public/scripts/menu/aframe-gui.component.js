...
var onAppendChildToContainer = function onAppendChildToContainer(elem, f) {
    ...
    var observer = new MutationObserver(function (mutations, me) {
        ...
        mutations.forEach(function (m) {
            if (m.addedNodes.length && !mutationTargets.has(m.target)) {
                f(m.target, m.addedNodes);
                mutationTargets.add(m.target);
            }
        });
    });
    ...
    init: function init() {
        ...
        onAppendChildToContainer(this.el, function (containerElement, addedChildren) {
            ...
            let handler = function (e) {
                ...
                addedChildren[0].removeEventListener("loaded", handler);
            };
            //addedChildren[0].addEventListener("loaded", handler);
        });
    },
    update: function update() {},
    tick: function tick() {},
